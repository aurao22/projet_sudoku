...
def reorder(myPoints, verbose=0):
...
def biggestContour(contours, verbose=0):
    biggest = np.array([])
    max_area = 0
    for i in contours:
...
def getPredection(boxes,model, verbose=0):
    result = []
    for image in boxes:
        ## PREPARE IMAGE
        ...
        if probabilityValue > 0.8:
            classIndex = convert_prediction_to_class(predictions, verbose=verbose-1)
...
def convert_prediction_to_class(one_picture_prediction, verbose=0):
...
def stackImages(imgArray,scale, verbose=0):
    rows = len(imgArray)
    cols = len(imgArray[0])
